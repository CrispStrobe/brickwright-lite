...
| D8 | `pc52-inductor-filter` is an RLC used as an RL bench | signals-rl-response (1 of the 3 D8 is counted against — the other two use pc52 as an RLC correctly) | **this pass** — sb3-creator `4512354`, a new `pc89-rl-step` | **no longer true** | the RL law now holds to 0.01 % across the whole response; the conflict that made a parameter fix unsafe is pinned as its own sentinel |
