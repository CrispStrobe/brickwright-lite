...
| Thing | Where |
|---|---|
| The extension, readable | `overlay/scratch-vm/src/extensions/crispstrobe/spikeprime/source.js` |
| The bundle the VM loads | `…/spikeprime/index.js`, generated by `scripts/spike/build-bundle.mjs` |
| The rename table | `overlay/scratch-vm/src/extension-support/spike-legacy-migration.js` |
...
### Why the source is a separate file from the bundle

The CrispStrobe extensions arrived as TurboWarp-unsandboxed modules that self-register
against a `Scratch` global, so the adapter runs their text through `new Function` and
every bundle holds its extension as one long JSON string. Fine for a vendored file nobody
edits; not fine for one we maintain, because a 7 000-line JSON string cannot be linted,
diffed or reviewed. `test/spike-bundle-sync.test.mjs` fails if the two drift, which is
what makes the readable file the real one rather than a stale copy of it.
...
| `spike-bundle-sync.test.mjs` | the readable source is the real program |
