...
| --- | --- | --- | --- |
| 1 | CI-only qualification harnesses for candidate Z80 and W65C02 cycle cores | pinned-source provenance; retire corpus; non-empty bus traces; reset/IRQ/NMI/HALT or WAI/STP vectors; snapshot-at-every-microstep replay hashes; bounded transfer receipts | in progress — Z80 passes the bounded pinned retire/timing corpus and native microstep gates; JSMoo is reproducibly rejected by snapshot, BBR-bus and WAI-wake defects; hosted Chromium receipt remains |
