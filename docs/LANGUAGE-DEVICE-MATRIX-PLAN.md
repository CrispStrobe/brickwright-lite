...
| Evidence for a sim cell rests on our own programs | third-party code that predates the emulator: ELKS 0.9.1 boots on the 8086 tier and mounts its root filesystem (bw-board `824d301`, banner at 1.31M instructions, root at 2.36M; BIOS ROM, uPD765, 8237 and 8259 driven together) — tier 2a in spirit, `needs: ['elks-image']`, `ciAvailable: false` (GPL-2, never vendored), so recorded, not standing | the 8086 `sim` cell cites it once T6 joins the census; the later userland panic is a separate open item and says nothing about the boot claim |
