...
| **ASM** | **N** hosted sdas8051 · reader **l** (L1) | **N** hosted avr-gcc · **l** | **N** hosted `rp2040`, runs in rp2040js (N4 done 2026-09-25) · **l** | **N** hosted `stm32f030`, runs in the stm32f0 core (N4) · **l** | **N** hosted `nrf52833`, routed from the ASM tab (N4) · **l** | **N** hosted ca65 · **l** | **N** hosted sdasz80 · **l** | **N** local, MASM **and** NASM (auto-detected; a Dialect selector in the ASM tab forces either, 2026-09-05) · **l** (L1 starts here) | native — (no emulator) |
...
**N1. Z80 C via SDCC `-mz80`.** Repo: stc-compiler, then lite. **BUILT 2026-09-05** on stc-compiler branch `lane/z80-c-target` (`6e40fb6e`, delegate, audited by lego-ac: 21 new tests, 432 repo tests green, ten mutation proofs). The vendored SDCC 4.0.0 already had the z80 port; what was missing was `share/sdcc/lib/z80` (crt0 + z80.lib), now vendored from the same .deb the fetch script uses. Map: ROM $0000–$7FFF, RAM $8000–$FFFF from `examples/z80-pd-bench/EXPECTED.md`; `--code-loc 0x0200 --data-loc 0x8000`; stock crt0 (jp init at $0000, SP at $0000 so the first push lands at $FFFE). Also fixed: `stages.py` dropped every exported `GR` symbol (affects 8051 too). **DONE 2026-09-25**: merged as stc-compiler #14 (`f1574b0`), deployed (a live compile returns a 32 KB ROM, first byte `C3`); lite half on branch `lane/matrix-n1-n4`: ▶ Run C on Z80 in the C tab (`data-testid="bw-run-c-z80"`, hands the ROM to the bench via `bw-asm-rom-ready`), z80 `pickerCompile: true`, the matrix cells shipped; `test/z80-c-bench.test.mjs` boots the example's program compiled as C and walks the light across `latch1.Q0`–`Q7`. The CP/M `.COM` route is still open (a separate row).
...
**N2. 8086 C — connect SmallerC to the assembler that already reads NASM.** Lite only. **DONE 2026-09-05** on `lane/language-device-matrix` (delegate, audited by lego-ac): 5 of 5 corpus programs assemble, `requestCBuild`/`cRouteFor` in `bw-asm/assemble-route.js`, ▶ Run C on 8086 in the C tab, an exit-code leak in the WASM glue fixed; `test/smallerc-to-i8086-asm.test.mjs` prints the tally on every run. Not done: a browser gate (box forbids playwright today), `float` support. **Port-I/O edge (added by the pin·i8086 lane):** SmallerC has no inline asm and no port intrinsic, so the C route supplies two primitives, `bw_outb(port, value)` and `bw_inb(port)`, injected into the image by `compileC8086` ONLY when the compiled body calls them (externs stripped). Their contract is SmallerC's cdecl: 16-bit args pushed right to left (first arg at `[bp+4]`), the caller cleans, a byte result in AX; their bodies are the same `OUT DX, AL` / `IN AL, DX` idiom `pseudocode-8086.js` emits inline. This is what lets emitted C reach the I/O-mapped 8255 (the 6502/Z80 peripherals are memory-mapped and need no such primitive).
...
the corpus count of programs excluded on i8086. **Exclusion rule, exact:** a program is blocked on i8086 iff generateC emits `static long` for it, which it does for any STORED number — pin and shift-register programs, which store none, compile (`repeat N` and `repeat until <cond>` too: their counter/condition store no long). **Step 1 landed 2026-09-07** (worker): `test/i8086-c-long-ceiling` pins SmallerC's "Unexpected token long" and the route's by-name refusal (`cUsesLong`, before the compiler); the C×8086 note in `capabilities.js` names the edge and N2b. **Corpus measured 2026-09-07** (worker): over 300 property-based programs retargeted to i8086, **8 of the 33 that get past the verb choke are blocked by the long rule** (all 8 by a single numeric variable); 25 compile; the other 267 are refused by the choke FIRST (they use verbs i8086 has not implemented — analog, pwm, motor, tone, servo, displays), which merely HIDES the ceiling. Device-independent (a numeric variable emits `static long` on every device), the REACH is **177 of 300 (59%)** programs store a number — 107 by a single variable, 60 by two, 10 by three-or-more — and each becomes long-blocked the moment its verbs gain an i8086 branch. The shipped bare-metal examples store numbers too. So: **8 of 33 admitted today, 177 of 300 as the choke lifts.** The `static long` grep is mechanical; x/y are sprite coordinates and emit no long, so they are excluded by construction.

**SmallerC long-in-.COM probe (bounded, 2026-09-07): NO — not via a flag.** `smallerc-wasm/compiler.js` compiles with `-seg16`, which `cgx86.c` makes 16-bit (`SizeOfWord = 2`) with **no 32-bit integer type** — smlrc's parser rejects `long` there (confirmed: "Unexpected token long"). The same binary DOES emit 32-bit code, but only WITHOUT `-seg16` (its huge/flat models), and those produce `.EXE`/flat images, not the tiny `.COM` the DOS bench `loadCom`s. So `long` and the `.COM` output are a mutually-exclusive codegen choice, not a runtime flag: a `long`-in-`.COM` would need either a SmallerC source patch giving `-seg16` a 32-bit type, or a 32-bit-model bench + loader — both larger than option (b). This confirms the lean below: build **(b)**, the emitter int-16 model with refuse-by-name. (2) The fix chosen by measurement between a SmallerC model
...
**N4. ARM assembly, hosted — extend the chain that exists.** Repo: stc-compiler, then lite. **stc-compiler half BUILT 2026-09-05** on branch `lane/arm-assemble-rp2040-stm32f030` (`a45ba77`, a second session): `rp2040` (cortex-m0plus, `pico-sram.ld`) and `stm32f030` (cortex-m0, `stm32f030-flash.ld`) beside `nrf52833` in the three maps the arm branch reads; router verified end to end (68 and 84 bytes of Intel HEX), 41/41 in `test_assemble.py`, each assertion mutation-proven. **DONE 2026-09-25**: merged as stc-compiler #13 (`11393d6`), deployed (live: 260 B stm32f030, 170 B rp2040); lite half on `lane/matrix-n1-n4`: `asmTargetForDevice` maps pico/rp2040 → `rp2040` and microbit/calliopemini → `nrf52833`, and ▶ Assemble & run hands the image to the debugger (`lib/bw-asm/arm-image.js`: Intel HEX → flash image at `0x08000000` for stm32f0, SRAM at `0x20000000` for rp2040js). `test/arm-asm-run.test.mjs`: PA5 toggles on the stm32f0 and GP25 on the rp2040js.
