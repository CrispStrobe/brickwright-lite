...
| **D26** | `20-shift-register-binary` shifts a constant zero: with `val = 128`, `bitand val 128 > 0` is false while `bitand val 128` alone is true | sb3-creator (compiler or referee — not isolated) | **1** | 7 | open — see PLAN.md |
