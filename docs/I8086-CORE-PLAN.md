...
| P16b — next experiment | **Emit one shared multi-root validator module.** Intern generated validator and `$ref` function identities across project/sprite SB2/SB3 roots so P16a's duplicated definition closures appear once. | Run a source-size preflight before CI and stop unless the 76,800-byte emitted win is plausible. Retain P16a's exact parser/error/ownership gates; add the GUI project corpus plus SB1 and `addSprite` VM boundaries. Require at least 20,480 encoded script bytes saved, no new request, and the standard 115% / 1 s / 100 ms runtime limits. |
