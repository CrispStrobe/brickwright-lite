...
| P17 — complete (valid eager baseline `34055140364`; accepted run `34055549914`) | **Demand-load the whole Sound tab on first selection, with intent prewarming.** One cached retryable `sound-tab.js` now owns 284,924 source / 198,558 emitted bytes across the editor, recorder, waveform/meter UI, `crispfxr-core`, audio helpers and WAV encoder. Initial JavaScript fell 4,543,936 → 4,351,060 bytes, a 192,876-byte reduction. Hover/focus starts the same request just ahead of selection without an idle or first-load fetch; late-loaded audio creates its shared context on first use. | The corrected eager gate waits for the published editing target and eight real Sound controls, not an unrelated panel or fixed delay: 113.6 ms with no long task. The accepted cold activation is 121.6 ms (+7.0%); three unchanged-production receipts measured 121.6–136.4 ms, so the runner-calibrated limits are 150 ms / 1 s / 100 ms. The gate also requires exactly one 198,558-byte request, matching VM/Redux targets, no loading residue and no page or console errors. Ownership, retry/failure source guards, corpus, unit/build, both browser shards, deploy and post-deploy GUI verification all passed. |
| P18 — evaluated, rejected (baseline `34056846253`; candidate `34059625658`) | **Demand-load the Connection modal body only when it opens.** The candidate reduced initial JavaScript 4,351,060 → 4,224,748 and preserved the micro:bit updater behind the same route so WebUSB remained in the click chain. | The deterministic named asset was 75,446 bytes, 1,354 below the 76,800-byte floor. Its full webpack named group was 128,139 bytes after a 52,693-byte vendor extraction, but the declared named-asset gate was not changed after measurement. Aborted-import recovery also failed to reach the required retry UI. Production stays eager; exact run/head/hash/asset/group evidence remains in the P18 receipt. |
| P19 — evaluated, stopped at attribution | **Keep scratch-storage's fetch worker out of boot until a parallel remote asset batch needs it.** Webpack does place the package's single 280,064-byte prebundled module entirely in the initial shared chunk, but that number includes the required storage API and direct fetch path. | The checked receipt pins webpack hash `30fa48f7e6bfb148c743`: the escaped inline worker, `FetchWorkerTool`, `InlineWorker`, and the entire `ProxyTool` total only 32,773 raw bytes, 44,027 below the 76,800-byte emitted floor. The referenced and decoded 22,345-byte fallback file is not emitted by the app. Package, nested/root stats, module ownership, asset path, malformed size and exact-boundary mutations fail closed. No candidate was built. |
| P20 — evaluated, rejected (candidate `c0893500f`; hosted run `34058754836`) | **Demand-load `scratch-sb1-converter` only after ordinary parsing rejects an input.** All 30 package modules were initial and totaled 87,364 source bytes; the candidate preserved the clean parser-rejection boundary. | The named chunk emitted only 43,171 bytes (14,086 gzip), 33,629 below the declared 76,800-byte floor. The hash-bound CI receipt is retained; the production split and temporary workflow hook were reverted without continuing to behavioral gates. Reconsider only if converter ownership grows materially or it can move with another legacy-only closure. |
