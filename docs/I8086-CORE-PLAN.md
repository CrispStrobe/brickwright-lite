...
| Debugger-only circuit deferral | **Deferred pending a dock-specific hosted receipt.** In `right`/`solo`, a stage portal exists but `CircuitDesigner` does not paint; the current lifecycle nevertheless loads the board, Circuit UI and part catalogue. Activate only if a cold Code-tab profile attributes at least 512 KiB encoded JavaScript, or 10% of cold-to-runner bytes, to those pre-paint edges in both desktop and mobile runs. Preserve File actions, starter projects, the visible Circuit tab and `top`/`off` preview behavior; require the byte reduction to exceed run-to-run spread and re-baseline first Circuit paint separately. |
