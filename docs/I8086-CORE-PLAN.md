...
| P17 — next | **Demand-load the whole Sound tab on first selection.** Static ownership suggests roughly 190–220 KiB of route-private source across the sound editor, recorder, waveform/meter UI, `crispfxr-core`, audio helpers and WAV encoder; shared scratch-audio and Library/AssetPanel stay eager. | First attribute the exact initial-only closure and stop below 76,800 emitted bytes (prefer 100 KiB because this is an interactive tab). A candidate needs one retryable request, stable loading/error UI and stale/unmount guards; prove waveform play/edit/undo/save, record, library, SoundFx and project round trip within the standard 115% / 1 s / 100 ms limits. |
