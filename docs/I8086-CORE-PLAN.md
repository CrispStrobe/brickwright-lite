...
| P10 — implemented; hosted receipt pending | **Split unused CodeMirror C, Python and JavaScript grammars.** Static grammar imports contribute 266,069 webpack-source bytes to the Code chunk although this journey selects ASM. Pseudocode, BASIC and ASM remain synchronous; C/C++, Python/MicroPython and JavaScript now have named demand-loaded chunks. Only the language compartment is reconfigured, with generation/dispose guards against stale arrivals and a usable plain-text fallback after load failure. | Require at least 250 KiB of optional grammar ownership and 100 KiB encoded isolation, with zero optional grammar fetches in all nine ASM receipts. The editor browser journey must fetch all three named chunks and receipt their encoded size. Rapid switching tests preserve text, selection and highlights. |
