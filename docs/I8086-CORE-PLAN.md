...
| P15b — next | **Demand-load the shared asset-library modal UI and its hidden Costume, Backdrop, Sound, Sprite and Extension container bodies as one measured boundary.** The current closure is about 110 KiB source after excluding the rejected tutorial runtime. Keep P6's already-lazy asset manifests and synchronous extension connection metadata unchanged. | Establish separate eager first-open baselines for all five libraries before changing imports. Prove hidden zero-fetch, exact item population/selection, concurrent-demand dedupe, cached reopen, failed-request retry and stale/unmount preservation for each route. Reject below 75 KiB emitted initial reduction or above the standard 115% / 1 s / 100 ms activation limits. |
