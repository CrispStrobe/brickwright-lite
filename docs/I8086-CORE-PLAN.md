...
| P4 — implemented; hosted proof pending | **Attribute Circuit-open updates before more React surgery.** The outer profiler stayed at 15 `CircuitDesigner` commits after both load batching and camera-fit coalescing, so it could not identify which state source survived. The benchmark now adds an opt-in nested `BoardCanvas` boundary and bounded marks for fit, resize, board-state, declaration and host updates. Raw/report v2/v3 receipts retain per-commit source rows and summarize both whole-startup and Circuit-open windows, including explicitly unattributed commits. | Normal builds retain the original child and allocate no probe or trace unless the benchmark installed its global before boot. The focused pure/source tests pass locally; the repeated hosted receipt is still required to account for the observed 15 commits and name a dominant source before P5 or another setter change begins. |
