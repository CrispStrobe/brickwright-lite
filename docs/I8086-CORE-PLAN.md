...
| P16c — next source-only preflight | **Measure structural equivalence before attempting validator graph deduplication.** Build a canonical, cycle-aware graph signature from generated function bodies, patterns, defaults, literal refs and referenced-node partitions; report equivalence classes without changing emitted code. | Stop without a candidate build unless exact structural classes predict at most 160 KiB raw generated source and every merge has identical schema/error behavior. Only then prototype canonical emission and restore P16a's differential parser, GUI/SB1/`addSprite`, ownership and encoded/runtime gates. Never merge merely similar `$ref` nodes or normalize observable AJV error paths. |
| P17 — queued after P16c | **Demand-load the whole Sound tab on first selection.** Static ownership suggests roughly 190–220 KiB of route-private source across the sound editor, recorder, waveform/meter UI, `crispfxr-core`, audio helpers and WAV encoder; shared scratch-audio and Library/AssetPanel stay eager. | First attribute the exact initial-only closure and stop below 76,800 emitted bytes (prefer 100 KiB because this is an interactive tab). A candidate needs one retryable request, stable loading/error UI and stale/unmount guards; prove waveform play/edit/undo/save, record, library, SoundFx and project round trip within the standard 115% / 1 s / 100 ms limits. |
| P18 — queued | **Demand-load the Connection modal body only when it opens.** The plausible private closure is 155–180 KiB source: modal UI plus DAP.js, universal-hex and micro:bit update helpers. Extension metadata and the native Bluetooth shim have other eager consumers and are not claimed. | Require exact initial-only attribution above the 76,800 emitted floor, zero hidden requests, and scanning/connect/cancel/error/retry, firmware update and rapid close/reopen coverage under the standard latency limits. |
| P19 — queued, attribution first | **Keep scratch-storage's fetch worker out of boot until a parallel remote asset batch needs it.** The package owns 280,064 initial source bytes while its authored source is much smaller, suggesting worker-loader inline/fallback duplication. Keep direct `FetchTool` available for project and startup requests. | Prove the inline worker is at least 76,800 emitted bytes, exclusively owned and unused by default startup before editing it. Preserve remote SB2/SB3 loads, parallel asset bytes/order, credentials and headers, worker-failure fallback, offline/Tauri stores and standard latency limits. |
