...
| P6 — in progress | **Audit and split startup chunks from evidence, not filenames.** Hosted run `33952110716` emitted the first raw webpack graph. Corrected analysis of its artifact measured 11.52 MiB uncompressed eager JavaScript (`2923`: 8.65 MiB; `gui`: 2.88 MiB). The largest module owners were scratch-vm 4.86 MiB, render fonts 1.31 MiB, asset-library data 0.72 MiB and text-encoding 0.60 MiB. A cold navigation through a running DOS program fetched 16.0 MiB across 20 scripts and exposed 94 unrelated module memberships; the narrower selection-to-running window was 1.01 MiB across six scripts with no unrelated modules, proving the broad graph had already loaded when Code/Debug mounted. The dedicated 22.1 KiB `bw-debug-i8086` chunk itself was non-initial and clean. The stats parser fix is `9897b30d0`; the evidence-backed eager split now awaits current-main hosted validation. | The profiling build alone emits compressed raw stats plus an owner report; the deployable production invocation remains unchanged. The candidate moves lazy extension data, render fonts, asset manifests, lesson/example catalogs and the redundant text-encoding tables out of eager scripts. Existing gates ratchet first-load bytes and deliberately abort a lazy extension chunk to prove one missing feature cannot empty the project. Do not claim the DOS journey clean until the early broad board/Circuit UI imports are removed and the causal receipt enforces zero unrelated modules. |
