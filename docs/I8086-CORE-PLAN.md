...
| P12 — active | **Import only the `react-virtualized` List entry used by list monitors.** One barrel import currently makes all 66 package modules initial: 350,770 source bytes, including unused Table, Masonry, Collection, MultiGrid and WindowScroller families. The direct ES `List` entry keeps the existing synchronous component and layout while retaining only List, Grid and their utility closure. | Hosted ownership must keep initial `react-virtualized` source at or below 140 KiB and prove the unused families absent. Existing monitor behavior plus a visible long-list scroll/edit/add/remove browser journey must remain green. Stop if package resolution changes behavior or emitted reduction is below 75 KiB. |
| P13 — queued after P12 | **Make SVG sanitation an upload-time capability without making rendering asynchronous.** `css-tree` and `mdn-data` contribute 572,453 initial source bytes through the broad `scratch-svg-renderer` barrel. Existing synchronous consumers can use narrow font, bitmap, load and serialize entries; only the SVG upload branch should demand a retryable named `svg-sanitizer` chunk before storage. | Ownership must show no initial renderer barrel, sanitizer, `css-tree` or `mdn-data`, and place both packages only in the non-initial sanitizer asset. Default rendering and Costume activation fetch none. Adversarial SVG upload fetches once, stores only the sanitized asset, makes no external request, displays and survives save/reload; a failed chunk stores nothing and retries. Apply the same 15% / 1 s / 100 ms activation limits and 75 KiB emitted stop rule. |
| P14 — queued after P13 | **Demand-load the remaining List/Grid closure on the first visible list monitor.** Keep the outer monitor geometry synchronous and load only its scrolling body with shared retry, stale/unmount guards and a same-size retry state. | Establish an isolated hosted first-list baseline. A 1,000-row saved monitor must retain position and dimensions, scroll and edit correctly, load the named asset once and recover from one failed request. Reject above 115% baseline, 1 s, 100 ms task time, or below 75 KiB emitted reduction. |
| P15 — queued after P14 | **Split hidden card and library UI from compact synchronous metadata.** Current eager library data is 151,396 source bytes; tutorial decks account for 95,893, with additional hidden card/modal UI. Preserve synchronous `initTutorialCard`, URL tutorial lookup and the extension connection metadata while hydrating full decks and visible modal bodies only on demand. | Deep-link `?tutorial=getStarted`/`all`, Tips, Costume, Backdrop, Sound and Extension journeys must populate and select correctly. Every retryable chunk needs cancel/retry and stale-completion proof. Reject any isolated slice below 75 KiB emitted or over the standard activation limits. |
| P16 — experiment only after P15 | **Precompile the scratch-parser schemas instead of naively lazy-loading AJV.** AJV is reached synchronously by every `loadProject` and `addSprite`, so `import()` would merely add a boot request. A CI-generated validator could potentially replace about 260 KiB of AJV/URI/schema traversal source, but only if it keeps the exact parser callback, errors, result shape and SB1 fallback. | Run differential valid/invalid/adversarial fixtures and the complete project corpus against both validators. Reject on any semantic difference or without a material net initial and encoded-byte win; never skip validation for bundled/default projects. |
