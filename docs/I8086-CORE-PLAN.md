...
| P9 — implemented; hosted receipt pending | **Move bundled examples from Code reveal to their no-device Tools control.** The currently accepted journey still fetches `pseudocode-examples.js` in all nine runs: 265,252 encoded bytes for a picker that an i8086 ASM session never opens. Code reveal now loads only CodeMirror; the shared example promise starts when Tools is open with no device, or when restored-game controls actually need source identification. | Require zero `pseudocode-examples.js` assets in all nine pre-Circuit receipts, then prove the Tools loading/retry state and known-game selection in the existing browser suite. Preserve retry after chunk failure, unmount/stale-request guards, restored game controls and the transition from an open hardware catalog to no device. |
| P10 — queued after P9 | **Split unused CodeMirror C, Python and JavaScript grammars.** Static grammar imports contribute 266,069 webpack-source bytes to the Code chunk although this journey selects ASM. | Require at least 250 KiB of optional grammar ownership and 100 KiB encoded isolation, with zero optional grammar fetches in the ASM receipt. Rapid language switching needs a generation guard and must preserve text, selection and highlights. |
| P11 — queued, higher-risk | **Install the scratch-paint reducer only when the Costume editor is demanded.** The existing editor component is lazy, but store construction synchronously imports its reducer and therefore puts 533,665 bytes of `@scratch/paper` plus 104,459 bytes of scratch-paint modules into initial JavaScript. | Activate only through a reducer manager using `store.replaceReducer`, before the lazy PaintEditor renders. Require zero initial Paper/paint ownership, preserved GUI/locales state, idempotent installation, a real Matrix-backed paint state and a full draw/save/reload costume round trip. Revert if first-Costume interactivity regresses by more than 15%, exceeds 1 s desktop, or adds a >100 ms long task. |
