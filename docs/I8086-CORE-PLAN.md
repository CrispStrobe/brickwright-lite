...
| P11 — implemented; hosted candidate pending | **Install the scratch-paint reducer only when the Costume editor is demanded.** The existing editor component was lazy, but store construction synchronously imported its reducer and therefore put 533,665 bytes of `@scratch/paper` plus 104,459 bytes of scratch-paint modules into initial JavaScript. Hosted baseline run `33967333844` measured the existing first-Costume PaperCanvas readiness at 390.5 ms with 50/55 ms activation tasks. The candidate uses a store-local reducer manager and shared retryable import, installs the real reducer through `store.replaceReducer` before rendering PaintEditor, and guards failed/stale/unmounted loads. | Require zero initial Paper/paint ownership, preserved GUI/locales state, idempotent installation, a real Matrix-backed paint state and the full vector/bitmap draw/save/reload round trip. The accepted 15% ceiling is 449.075 ms, with the separate 1 s absolute and 100 ms long-task limits. Public `src/index.js` keeps its synchronous `guiReducers.scratchPaint` API; the deployed playground entry is the optimized graph. |
