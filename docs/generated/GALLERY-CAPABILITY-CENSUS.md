...
Generated by `scripts/sync-gallery-pins.mjs` from immutable `CrispStrobe/extensions@4e97951c6059edb24eefe313a71697e3e9560948`. Do not hand-edit.

Denominator: **128/128 URL-loaded pins**; bundled built-ins are excluded. Runtime-proven workers: **24**; zero-requirement candidates awaiting runtime proof: **1**; deferred: **103**. Only the worker cohort claims runtime compatibility.
...
| box2d | deferred | dom, runtime, fetch-import | reads stage dimensions and requests redraw/target updates through Scratch.vm.runtime; its document/window scanner hits are license comments and commented CommonJS examples, not ambient DOM dependencies |
...
| CrispStrobe/bitops | candidate | none | zero statically detected ambient requirement |
| CrispStrobe/brickwright_tts | deferred | dom, runtime, nested-worker | speaks through window.speechSynthesis and SpeechSynthesisUtterance, falls back to AudioContext/webkitAudioContext for tones, and spawns a Worker for the local recogniser driver; the restricted worker exposes none of those, and it also reads Scratch.vm.runtime |
...
| CrispStrobe/devices | deferred | runtime | reads the host's `stc` and `circuitBoard` seams off Scratch.vm.runtime to drive the simulated parts; the restricted worker exposes no VM runtime and therefore no board |
...
| CrispStrobe/legospike_turbowarp_transpile | deferred | dom, runtime, websocket, web-bluetooth | runs an executable locale diagnostic against navigator, document, window storage, and Scratch.vm.runtime.getLocale |
...
| CrispStrobe/microbitplus | deferred | runtime | resolves the open circuit through Scratch.vm.runtime.circuitBoard on every board-facing block; the restricted worker exposes no VM runtime and therefore no board |
...
| CubesterYT/Webhooks | deferred | runtime, fetch-import | drives runtime.extensionManager and persists through runtime.extensionStorage, and subscribes to VM lifecycle with runtime.on; its Scratch.fetch posts are worker-safe but the extension-manager and storage seams are not |
...
| lab/video-sprites | deferred | dom, runtime, fetch-import | creates DOM elements with document.createElement and reaches the renderer through runtime.renderer, runtime.ioDevices and runtime.requestRedraw; the restricted worker exposes neither a document nor the renderer |
...
| NishiOwO/dectalk | deferred | dom, runtime | installs a window.onDECtalkAudioCallback global for its emscripten synthesiser and plays through Scratch.vm.runtime.audioEngine.audioContext; the restricted worker exposes neither a window to hang the callback on nor the audio engine |
...
| SamuelLouf/Geolocation | deferred | runtime | reads navigator.geolocation, navigator.permissions and navigator.userAgent, and wakes hats via Scratch.vm.runtime.startHats; the restricted worker exposes no navigator geolocation and no VM runtime |
...
| SharkPool/Messages-Plus | deferred | runtime | schedules and inspects threads directly on Scratch.vm.runtime -- _pushThread, _restartThread, isActiveThread, allScriptsByOpcodeDo and target lookups; the restricted worker exposes no VM runtime and therefore no thread table |
| SharkPool/Tune-Shark-V3 | deferred | runtime | pushes threads with runtime._pushThread, scans scripts with runtime.allScriptsByOpcodeDo and sequences against runtime.audioEngine and runtime.currentStepTime; its Scratch.canFetch check is worker-safe but the runtime and audio-engine access is not |
...
| TheShovel/CanvasEffects | deferred | dom, runtime | explicitly requires unsandboxed execution, Scratch.vm runtime lifecycle, and renderer resize and canvas styling |
