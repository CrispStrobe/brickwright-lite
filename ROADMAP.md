...
| `i8086-asm.js` | writing 8086 assembly in the ASM tab and running it on the extracted machine | **a local assemble path.** This is the one entry on this list whose blocker is not "a feature has not landed yet". The ASM tab assembles through HOSTED ca65 and sdasz80; neither knows the 8086, so no amount of tab work reaches an 8086 target through the existing route. The 8086 either gets a local, in-browser assembler or it gets no ASM tab at all | a decision, not a task: one tab with two assemble paths (hosted for 6502/Z80, local for 8086) is a real inconsistency, and the alternative — moving everything local — is a much larger change. Worth noting the local path is strictly better where it applies: no network, and the assembler is differentially tested against MASM 1.10 itself |
