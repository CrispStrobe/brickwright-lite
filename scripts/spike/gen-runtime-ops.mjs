...
/**
 * Rendered as a JS module rather than a .json file on purpose: Node's ESM
 * loader requires `with {type: 'json'}` on a JSON import and webpack does not,
 * so a .json here would have to be imported two different ways for the browser
 * build and the unit suite. A module is read identically by both.
 */
export const render = entry =>
    `${HEADER}export default ${JSON.stringify(entry, null, 4)};\n`;

const HEADER = `// GENERATED by scripts/spike/gen-runtime-ops.mjs — do not edit by hand.
//
// The unified spikeprime block surface, in sb3-creator's runtime-registry
// shape. Merged over the pinned vendored registry at
// lib/sb3-creator-register-art.js; see that file and
// test/spike-runtime-registry.test.mjs for why it is merged rather than
// generated into the vendored file itself.
`;

if (import.meta.url === `file://${process.argv[1]}`) {
    const entry = buildEntry();
    const dest = resolve(root, 'overlay/scratch-gui/src/lib/spike-runtime-ops.js');
    writeFileSync(dest, render(entry));
