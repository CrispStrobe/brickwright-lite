...
export function censusEntry (slug, source) {
    ...
    return {
        ...
        migration: {
            status: capabilities.length ? 'deferred' : 'candidate',
            reason: capabilities.length ? `static scan requires review: ${capabilities.join(', ')}` : null
        }
    };
}
...
export function renderCensusReport (document) {
    ...
    return `# Gallery capability census\n\nGenerated by \`scripts/sync-gallery-pins.mjs\` from immutable ` +
        ...
        `Zero-requirement candidates: **${compatible}**; deferred: **${rows.length - compatible}**. ` +
        `This static census does not claim runtime compatibility.\n\n` +
        `| Capability | Pins |\n|---|---:|\n` +
