...
export function renderCensusReport (document) {
    ...
    return `# Gallery capability census\n\nGenerated by \`scripts/sync-gallery-pins.mjs\` from immutable ` +
        ...
        // BOTH numbers come from the same set, because the claim is that every
        // pinned extension is censused with no silent remainder. The second was
        // a literal 120, which was invisible while there were 120 pins and
        // printed "124/120" the moment four more were added.
        `Denominator: **${rows.length}/${rows.length} URL-loaded pins**; bundled built-ins are excluded. ` +
