...
| --- | --- | --- | --- |
| D2 hosted compiler | 12 | lite | **Re-audited 2026-08-30: the advertised escape hatch is not yet a compiler path.** The bundled SDCC distribution contains headers and three executables but no mcs51 runtime libraries. The executables also have separate Emscripten filesystems while `compiler.js` hands `/input.rel` to `sdld` without copying it. Finally, symbol extraction was a stub, so even a successful image could not start the debugger. The protocol-004 CDB parser is now implemented and fail-closed in `sdcc-wasm/symtab.js`, with address-space, yield-map and linked-line mutation coverage. D2 remains open until a rebuilt bundle includes the runtime libraries, explicitly transfers every stage artifact (or uses one shared worker filesystem), and passes an offline browser compile+debug gate. Do not make the flag default before that gate. |
