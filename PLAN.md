...
| 3b | LED polarity in the generated benches | Unclaimed. Ten of the eleven device families ship benches whose LED wiring contradicts the program's `ACTIVE LOW`/`ACTIVE HIGH` declaration: 230 of 394 declared-output LEDs are inverted, measured against `e3a1f960e` by `scripts/led-polarity-census.mjs` and written up in `docs/EXAMPLE-CORPUS-FINDINGS.md`. `stc12c5a60s2` is clean because it is the authored device; `stc89c52rc` and `stc15f2k60s2` are not, so this is not an 8051-versus-ports split. **The first deliverable of this lane is the PRODUCER'S NAME, not a diff.** Rewriting 230 bench files without knowing what wrote them means the next regeneration reproduces them, and a corpus-wide rewrite buys nothing. The obvious candidate is already eliminated by asking it rather than reading it: both netlist inferrers branch on `pin.activeLow` correctly today and produce cathode-to-pin for an active-low 8051, Uno and Pico alike, and the shipped benches seat a real board part where `inferNetlist` emits a generic `MCU`, so they did not come from there. "Hand-authored" or "a tool that no longer exists" is a fine answer and changes the repair from fixing a generator to fixing the data and adding a gate. Acceptance: the producer named, the benches corrected, and the census re-run at the new sha — nothing is fixed until that number moves. The census then becomes a gate and ratchets downward only. |
