...
| 3b | LED polarity: the retarget drops ACTIVE LOW where it is the lesson | Unclaimed. 52 of 449 decidable declared-output LEDs are inverted, in 4 examples, measured against `e3a1f960e` and unchanged after the pin bump at `fec0a9edf`, by `scripts/led-polarity-census.mjs` and written up in `docs/EXAMPLE-CORPUS-FINDINGS.md`. **That write-up keeps three earlier wrong versions of this number and what each got wrong** — an authored-instead-of-retargeted comparison, a by-name instead of by-net LED match, and silently dropped undecidable readings — because each was a claim that outran what had been checked. Two defects. (1) `06-active-low-high`, `32-source-vs-sink` and `46-port-overcurrent`, 49 rows on AVR and Pico: all three exist to teach SINKING, all three ship an authored circuit so `authored-transform.mjs` keeps the sink wiring, and `retargetPseudocode` drops the clause. Right for `01-blink`, wrong here; the obvious rule keyed on mixed-polarity declarations was measured and withdrawn, since it selects two unrelated examples and misses `46` entirely. (2) `arduino-sk-p15-hacking-buttons` pin `opto` on all three 8051 parts including the authored one, no retarget involved: a straight data defect. Producer identified: `CrispStrobe/sb3-creator` `scripts/gen-device-benches.mjs`; both netlist inferrers were asked directly and neither is wrong. **Extending coverage is a PREREQUISITE, not a follow-up**: of 764 declared output pins the census decides 449, with 139 carrying no discrete LED and 176 answering neither way, so a repair validated against 59% of the population is not validated. Acceptance: coverage extended, both defects fixed at their own layer, and the census re-run at the new sha reading zero. The census then becomes a gate and ratchets downward only. |
