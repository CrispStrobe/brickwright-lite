...
| 3b | LED polarity in the generated benches | Unclaimed. Ten of the eleven device families ship benches whose LED wiring contradicts the program's `ACTIVE LOW`/`ACTIVE HIGH` declaration: 230 of 394 declared-output LEDs are inverted, measured against `e3a1f960e` by `scripts/led-polarity-census.mjs` and written up in `docs/EXAMPLE-CORPUS-FINDINGS.md`. `stc12c5a60s2` is clean because it is the authored device; `stc89c52rc` and `stc15f2k60s2` are not, so this is not an 8051-versus-ports split. Both netlist inferrers branch on `pin.activeLow` correctly today and the shipped benches seat a real board part where `inferNetlist` emits a generic `MCU`, so the first step is identifying what actually produced them rather than assuming. Acceptance: the producer named, the benches regenerated or corrected, and the census reading taken again at the new sha — nothing is fixed until that number moves. |
