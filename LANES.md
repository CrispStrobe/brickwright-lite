...
| lessons post-repair re-check | lessons-recheck | 2026-08-24 | Re-measuring what the calculator repair (lite `39b83a1f9`) invalidated. **The premise is false for this tree, and that is the finding:** every wave result whose cause is now fixed had ALREADY been re-worded on 2026-08-24 by the sessions that fixed it, and every checkpoint still carrying a workaround has a cause still open, each asserted by a green sentinel. Table: `docs/POST-REPAIR-RECHECK.md`. What the pass found instead: **the calculator repair was half of one** — its arming loop passes `false` as the pull's RAIL and a quasi pin idles HIGH, so 22 of the corpus's 67 wired controls (the whole 8051 side, incl. 3 Wave 5 benches) still could not move their own pin. 43 → 22 → 1, gated. Then D10 and D11 closed. **Rule 5:** moves lite's vendored `sb3-creator` — the driver change lands in every generated simulator program, `pc50-two-stage-rc`'s capacitors and `43-rc-timing`'s parts changed. **Collision, recorded:** pushing D10 left lite CI red and a parallel session repaired it (`e51bda00a`, better than mine — kept). Their restoration of `measurement-rc-cursors` was wrong by 1.4 V (one discharge time constant is 1.88 V, not the 0.45 V floor) and left `signals-rc-response`'s hint denying a discharge path the bench now has; both fixed here. The lesson is mine: run the gate a change touches BEFORE the push, not in the suite after it. |
