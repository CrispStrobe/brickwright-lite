...
| lessons post-repair re-check | lessons-recheck | 2026-08-24 | The calculator repair (lite `39b83a1f9`) invalidated review findings: any Wave 1-7 checkpoint whose text was WORDED AROUND a dead control / unresponsive keypad / missing play mode / the shift register / Arduino fade pins was worded around a defect that no longer exists. Enumerating every such finding, re-measuring it on the current build, restoring the wording where the workaround is now false, then closing the still-open rows of `docs/WAVE-OPEN-DEFECTS.md` hardest-first. Branch `review/post-repair-recheck`. |
...
| lite fetch pinning | bw-audit | 2026-08-24 | Handed over from the coordinating session. My four findings for THIS repo, plus what the sweep turned up around them. Denominator: **65 sites resolve an external name to content; 55 named a mutable object; now 1**, stated not hidden. Branch `fix/lite-fetch-pinning`, pushed. **Rule 5 — this changes what other repos' gates load in three ways:** (a) all 48 `uses:` are now 40-hex shas, so a workflow copied from here carries pins, not tags; (b) `scripts/lib-pin.mjs` is new and the four remote syncs now FAIL rather than falling back to a branch name when the commits API is unreachable — a sync in an offline runner that used to 'work' will now stop; (c) `build.yml`'s staleness guard compares against `vendor-pins.json` instead of upstream HEAD, so it no longer warns merely because an upstream moved. No pin VALUE moved. |
| schematic viewer, third pass | bw-circuit-ui | 2026-08-24 | The viewer audited a THIRD time, as a measurement with denominators over all 2,098 shipped circuits. Both previous passes measured only `projection.wires`; a symbol's OWN copper (art paths, generic-box pin leaders) is drawn by `schematic-svg.js` from the symbol description and never appears there, so no class could see it. Measured: **403 drawn pins across 109 circuits sit where their symbol's artwork does not reach** — a seven-segment display draws two whiskers at y=0 and eight wires arrive at eight points that touch nothing. Also carrying the owner's finding that OUR net solver is more permissive than EasyEDA's (a T-joint with no junction dot connects for us, crosses for them), as an import warning. Branch `fix/schematic-audit-numbers`. |
