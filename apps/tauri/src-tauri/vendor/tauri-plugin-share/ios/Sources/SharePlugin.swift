			...
			DispatchQueue.main.async {
					...
					if let scene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
					   let rootVC = scene.windows.first(where: { $0.isKeyWindow })?.rootViewController {
						// UIActivityViewController is a popover on iPad. UIKit raises an
						// exception and aborts the app unless it has a native anchor.
						// This command originates in WebKit, not a UIBarButtonItem, so use
						// the centre of the active view as a stable, arrowless anchor.
