//! macOS Bluetooth-Classic (RFCOMM/SPP) transport — Rust side of the IOBluetooth
//! shim in `bt_macos.m`. Bridges the ScratchLink BT JSON-RPC surface (as spoken
//! by scratch-vm `io/bt.js`) to native RFCOMM, for EV3 + legacy-firmware SPIKE.
//!
//! Requests:  discover{majorDeviceClass,minorDeviceClass} / connect{peripheralId} / send{message}
//! Notifications: didDiscoverPeripheral / didReceiveMessage (base64 byte stream)
//!
//! Scratch drives one BT peripheral at a time, so a single global session (in the
//! ObjC shim) plus one `STATE` slot here is sufficient. The C callbacks fire on
//! the shim's run-loop thread and push frames to the active WebSocket via the
