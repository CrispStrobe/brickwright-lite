...
const L10N = {
    en: {
        ...
        cNote: 'C for the STC12 / 8051. Paste your own firmware and press ⇦ To blocks, or compile it to a .hex with stc-compiler.vercel.app. On the 8086 the C compiles and runs locally (SmallerC + the in-browser assembler); drive the 8255’s pins with the port-I/O primitives bw_outb(port, value) and bw_inb(port) — cdecl, 16-bit args, caller cleans — the same ones the generated C uses. Keep scalar variables and parameters unsigned (word-wide), not unsigned char: reading a byte value makes SmallerC emit MOVZX, an 80386 instruction the 8086 assembler refuses.',
    ...
    de: {
        ...
        cNote: 'C für den STC12 / 8051. Eigene Firmware einfügen und „⇦ Zu Blöcken” drücken, oder auf stc-compiler.vercel.app zu .hex kompilieren. Auf dem 8086 wird das C lokal kompiliert und ausgeführt (SmallerC + der In-Browser-Assembler); die Pins des 8255 werden mit den Port-I/O-Primitiven bw_outb(port, value) und bw_inb(port) angesteuert — cdecl, 16-Bit-Argumente, der Aufrufer räumt auf — dieselben, die das erzeugte C nutzt. Skalare Variablen und Parameter unsigned (wortbreit) halten, nicht unsigned char: das Lesen eines Byte-Werts lässt SmallerC MOVZX erzeugen, eine 80386-Instruktion, die der 8086-Assembler ablehnt.',
