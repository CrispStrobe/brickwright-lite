"use strict";
(self["webpackChunkGUI"] = self["webpackChunkGUI"] || []).push([["src_lib_bw-board_z80-adapter_js"],{

/***/ "./src/lib/bw-board/z80-adapter.js":
/*!*****************************************!*\
  !*** ./src/lib/bw-board/z80-adapter.js ***!
  \*****************************************/
...
/* harmony export */   createZ80Adapter: () => (/* binding */ createZ80Adapter),
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
/* harmony import */ var _z80_machine_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./z80-machine.js */ "./src/lib/bw-board/z80-machine.js");
/* harmony import */ var _devices_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./devices.js */ "./src/lib/bw-board/devices.js");
/* harmony import */ var _ps2_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./ps2.js */ "./src/lib/bw-board/ps2.js");
/**
...
function createZ80Adapter() {
  var _opts$config, _opts$romAt;
  let opts = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  const config = (_opts$config = opts.config) !== null && _opts$config !== void 0 ? _opts$config : opts.cpm ? _z80_machine_js__WEBPACK_IMPORTED_MODULE_0__.CPM64K : _z80_machine_js__WEBPACK_IMPORTED_MODULE_0__.SEARLE;
  let board = null;
  const stats = {
    serialCount: 0,
    advanceToCount: 0
  ...
  const machine = new _z80_machine_js__WEBPACK_IMPORTED_MODULE_0__.Z80Machine(config, {
    onSerial(byte, tMs) {
    ...
    onBufferRead(chipName) {
      ...
      for (let bit = 0; bit < 8; bit++) {
        ...
        const level = board.readPin("".concat(chipName, ".").concat(group, "a").concat(pin));
        if (level) value |= 1 << bit;
      }
      return value;
    }
  });
  if (opts.rom) machine.load(opts.rom, (_opts$romAt = opts.romAt) !== null && _opts$romAt !== void 0 ? _opts$romAt : 0);
  ...
  function seatLatchPins() {
    ...
    for (const lc of latchChips) {
      ...
      if (!chip) continue;
      const value = (_chip$value = chip.value) !== null && _chip$value !== void 0 ? _chip$value : 0;
      for (let bit = 0; bit < 8; bit++) {
        board.setPin("".concat(lc.name, ".Q").concat(bit), 'pushpull', !!(value & 1 << bit));
      }
    }
  }
  ...
  function bridgePS2(b) {
    ...
    for (const part of b.parts) {
      ...
      // each IN instruction.
      const cap = new _ps2_js__WEBPACK_IMPORTED_MODULE_2__.PS2Capture(state._kbd, {
        onByte: byte => {
      ...
      if (bufferName) {
        ...
        if (chip && typeof chip.read === 'function') {
          ...
          chip.read = () => {
            var _cap$_lastByte;
            return (_cap$_lastByte = cap._lastByte) !== null && _cap$_lastByte !== void 0 ? _cap$_lastByte : origRead();
          };
        }
      }
      machine.attachDevice("ps2_".concat(part.id), cap);
    }
  }
    ...
    machine.pcTraps.set(BDOS, () => {
      switch (cpu.c) {
        ...
        case 2:
          ...
          break;
        case 6:
          if (cpu.e === 0xff) setA(rxKeys.length ? rxKeys.shift() : 0);else if (cpu.e === 0xfe) setA(rxKeys.length ? 0xff : 0);else if (cpu.e === 0xfd) {
            if (!rxKeys.length) {
              wait();
              return 40;
            }
  ...
  return {
    ...
    load(bytes, at) {
      machine.load(bytes, at);
    },
    attachBoard(b) {
      var _opts$pc;
      board = b;
      bridgePS2(b);
      machine.cpu.pc = (_opts$pc = opts.pc) !== null && _opts$pc !== void 0 ? _opts$pc : opts.cpm ? 0x0100 : 0;
    ...
    sendSerial(byte) {
      if (opts.cpm) {
        rxKeys.push(byte & 0xff);
        return true;
      }
      for (const chip of Object.values(machine.chips)) {
        if (typeof chip.rxPush === 'function') {
          chip.rxPush(byte & 0xff);
          return true;
        }
      }
      return false;
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (createZ80Adapter);
