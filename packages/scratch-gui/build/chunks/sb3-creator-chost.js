"use strict";
(self["webpackChunkGUI"] = self["webpackChunkGUI"] || []).push([["sb3-creator-chost"],{

/***/ "./src/lib/sb3-creator-chost.js":
/*!**************************************!*\
  !*** ./src/lib/sb3-creator-chost.js ***!
  \**************************************/
...
/* harmony export */   "default": () => (/* binding */ cHostToPseudocode)
/* harmony export */ });
...
const STRUCTURAL = new Set(['stage', 'sprite', 'sprite_shape', 'local', 'local_list', 'costume', 'sound', 'defblock', 'global_var', 'global_list']);
  ...
  for (let i = m[0].length - 1; i < body.length; i++) {
    if (body[i] === '(') depth++;else if (body[i] === ')') {
      ...
      if (depth === 0) {
        ...
        return {
          name: m[1],
          args: splitArgs(body.slice(m[0].length, i))
        };
      }
    }
  }
...
class Reader {
  ...
  name(id) {
    ...
    if (this.params && this.params.has(id)) {
      return this.params.get(id) === '%b' ? "<".concat(id, ">") : "(".concat(id, ")");
    }
    const bare = (0,_sb3_creator_scratchruntime_js__WEBPACK_IMPORTED_MODULE_0__.stripSpritePrefix)(id);
  ...
  expr(text) {
    ...
    if (bin) return "(".concat(this.expr(bin.left), " ").concat(bin.op, " ").concat(this.expr(bin.right), ")");
    ...
    this.warn("cannot read expression ".concat(t));
    return '""';
  }
  balanced(t) {
    let d = 0;
        ...
        for (const op of ops) {
          if (t.startsWith(op, i) && i > 0) {
            ...
            const left = t.slice(0, i).trim(),
              right = t.slice(i + op.length).trim();
  ...
  callExpr(call, whole) {
    ...
    switch (name) {
      ...
      case 'bw_s':
        return this.expr(args[0]);
      case 'bw_join':
        return "(".concat(this.expr(args[0]), " join ").concat(this.expr(args[1]), ")");
      case 'bw_letter':
        return "(letter ".concat(this.expr(args[1]), " of ").concat(this.expr(args[0]), ")");
      case 'bw_length':
        return "(length of ".concat(this.expr(args[0]), ")");
      case 'bw_contains':
        return "(".concat(this.expr(args[0]), " contains ").concat(this.expr(args[1]), ")");
      case 'bw_mod':
        return "(".concat(this.expr(args[0]), " mod ").concat(this.expr(args[1]), ")");
      case 'bw_random':
        return "(pick random ".concat(this.expr(args[0]), " to ").concat(this.expr(args[1]), ")");
      case 'bw_mathop':
        return "(".concat(unquote(args[0]), " of ").concat(this.expr(args[1]), ")");
      case 'bw_cmp':
        return "".concat(this.expr(args[0]), " ?cmp? ").concat(this.expr(args[1]));
      case 'bw_list_item':
        return "(item ".concat(this.expr(args[1]), " of ").concat(this.listName(args[0]), ")");
      case 'bw_list_index':
        return "(item # of ".concat(this.expr(args[1]), " in ").concat(this.listName(args[0]), ")");
      case 'bw_list_length':
        return "(length of ".concat(this.listName(args[0]), ")");
      case 'bw_list_contains':
        return "(".concat(this.listName(args[0]), " contains ").concat(this.expr(args[1]), ")");
      case 'round':
        return "(round ".concat(this.expr(args[0]), ")");
      case 'floor':
      case 'ceil':
      case 'sqrt':
      case 'fabs':
        return "(".concat({
          ...
          sqrt: 'sqrt',
          fabs: 'abs'
        }[name], " of ").concat(this.expr(args[0]), ")");
      case 'tgamma':
        return "(factorial of ".concat(this.expr(String(args[0]).replace(/\s*\+\s*1$/, '')), ")");
      case 'pow':
        return "(".concat(this.expr(args[0]), " to the power of ").concat(this.expr(args[1]), ")");
      case 'fmin':
        return "(min of ".concat(this.expr(args[0]), " and ").concat(this.expr(args[1]), ")");
      case 'fmax':
        return "(max of ".concat(this.expr(args[0]), " and ").concat(this.expr(args[1]), ")");
      case 'bw_multiple':
        return "".concat(this.expr(args[0]), " is multiple of ").concat(this.expr(args[1]));
      case 'bw_pi':
        return '(pi)';
      case 'bw_e':
        return '(e)';
      case 'bw_sumdigits':
        return "(sum of digits of ".concat(this.expr(args[0]), ")");
      case 'bw_ask':
        return 'answer';
      default:
        break;
    ...
    this.warn("cannot read call ".concat(name, "() in ").concat(whole));
    return '""';
  }
  listName(ref) {
    return this.name(String(ref).replace(/^&/, '').trim());
  }

  /** An arrays_* call back through the same table the Python reader uses. */
  arrays(cname, args) {
    const r = (0,_sb3_creator_scratchruntime_js__WEBPACK_IMPORTED_MODULE_0__.arraysCallToPseudo)(cname.slice('arrays_'.length), args.map(a => this.expr(a)));
    if (r) return r.text;
    this.warn("unknown ".concat(cname));
    return null;
  }
  ...
  scratch(cname, args) {
    ...
    const r = (0,_sb3_creator_scratchruntime_js__WEBPACK_IMPORTED_MODULE_0__.scratchCallToPseudo)(method, args.map(a => this.expr(a)));
    if (r) return r.text;
    this.warn("unknown scratch_".concat(method));
    return null;
  }
  ...
  cond(text) {
    ...
    if (m = /^bw_cmp\s*\(([^]*)\)\s*(==|!=|<=|>=|<|>)\s*0$/.exec(t)) {
      ...
      return "".concat(this.expr(a), " ").concat(op, " ").concat(this.expr(b));
    }
    if (m = /^bw_n\s*\(([^]*)\)\s*!=\s*0$/.exec(t)) return this.expr(m[1]);
    if (t.startsWith('!')) return "not ".concat(this.cond(t.slice(1)));
    const bin = this.splitBinary(t);
    if (bin && (bin.op === 'and' || bin.op === 'or')) {
      return "".concat(this.cond(bin.left), " ").concat(bin.op, " ").concat(this.cond(bin.right));
    }
    return this.expr(t);
  }
}
 ...
 */
function cHostToPseudocode(source) {
  const r = new Reader();
  ...
  for (const raw of lines) {
    const line = raw.trim();
    if (line.startsWith('static void bw_structure')) {
      inStruct = true;
      continue;
    }
    if (inStruct) {
      ...
      if (call && call.name.startsWith('scratch_')) {
        struct.push({
          m: call.name.slice('scratch_'.length),
          a: call.args.map(unquoteArg)
        });
      }
    }
  }
  ...
  const out = [];
  for (const s of struct) {
    if (s.m === 'global_var') out.push("GLOBAL ".concat(s.a[0]));else if (s.m === 'global_list') out.push("GLOBAL LIST ".concat(s.a[0]));
  }
  ...
  for (const s of struct) {
    if (s.m === 'stage') sections.push({
      ...
      if (s.m === 'sprite_shape') sec.shape.push("  SHAPE ".concat(s.a[1]));
      sections.push(sec);
    } else if (sections.length) {
      const cur = sections[sections.length - 1];
      if (s.m === 'local') cur.locals.push("  LOCAL ".concat(s.a[0]));else if (s.m === 'local_list') cur.lists.push("  LOCAL LIST ".concat(s.a[0]));else if (s.m === 'costume') cur.costumes.push("  COSTUME ".concat(s.a[0]));else if (s.m === 'sound') cur.sounds.push("  SOUND ".concat(s.a[0]));
    }
  }
  ...
  while (i < lines.length) {
    ...
    if (fn && lines[i + 1] && lines[i + 1].trim() === '{' && fn[1] !== 'bw_structure') {
      ...
      if (section && !emitted.has(sidx)) {
        emitted.add(sidx);
        out.push('', section.head, ...section.shape, ...section.locals, ...section.lists, ...section.costumes, ...section.sounds);
      }
      const indent = section ? 1 : 0;
      r.params = null; // scoped to one DEFINE; header() sets it
      out.push('', ...pending.map(c => '  '.repeat(indent) + c), ...header(r, fn[1], fn[2], body).map(h => '  '.repeat(indent) + h), ...stmts(r, body, indent + 1));
      pending = [];
      continue;
    }
    if (/^int main\s*\(/.test(line)) {
      const {
        next
      } = takeBlock(lines, i + 1);
      i = next;
    ...
    if (line.startsWith('//')) pending.push('#' + line.slice(2));else if (line && !line.startsWith('/*') && !line.startsWith('*')) pending = [];
    i++;
  }
  for (const w of r.warnings) out.unshift("# ".concat(w));
  return out.join('\n').replace(/\n{3,}/g, '\n\n').trim() + '\n';
}
function unquoteArg(a) {
  const m = /^bw_str\s*\(\s*"((?:[^"\\]|\\.)*)"\s*\)$/.exec(String(a).trim());
  if (m) return JSON.parse("\"".concat(m[1], "\""));
  const n = /^bw_num\s*\(\s*(-?[\d.]+)\s*\)$/.exec(String(a).trim());
  return n ? n[1] : String(a).trim();
}
function sanitize(name) {
  return String(name).replace(/[^A-Za-z0-9_]+/g, '_').replace(/^_+|_+$/g, '');
}
...
function takeBlock(lines, open) {
  ...
  for (; i < lines.length; i++) {
    ...
    if (depth === 1 && /^\}\s*else\s*\{$/.test(t)) return {
      body,
      next: i,
      hasElse: true
    };
    ...
    depth += opens - closes;
    if (depth === 0 && i > open - 1 && (opens || closes)) {
      i++;
      break;
    }
  }
...
function header(r, cname, params, body) {
  const bare = (0,_sb3_creator_scratchruntime_js__WEBPACK_IMPORTED_MODULE_0__.stripSpritePrefix)(cname).replace(/_\d+$/, '');
  ...
  if (def) {
    ...
    const label = String(proccode).replace(/%[sb]/g, tok => {
      ...
      r.params.set(nm, tok);
      return tok === '%b' ? "<".concat(nm, ">") : "(".concat(nm, ")");
    });
    return ["DEFINE ".concat(warp ? 'FAST ' : '').concat(label, ":")];
  }
  ...
  let m;
  if (m = /^when_(.+)_key$/.exec(bare)) return ["WHEN ".concat(m[1].replace(/_/g, ' '), " key pressed:")];
  if (m = /^on_(.+)$/.exec(bare)) return ["WHEN I receive \"".concat(m[1].replace(/_/g, ' '), "\":")];
  r.warn("unrecognised function ".concat(cname, " \u2014 read as a flag script"));
...
function stmts(r, lines, depth) {
  ...
  while (i < lines.length) {
    ...
    if (m = /^for \(long \w+ = \(long\)(.+); \w+-- > 0; \) \{$/.exec(line)) {
      openBlock("REPEAT ".concat(r.expr(m[1]), ":"));
      continue;
    }
    if (m = /^while \(!(.+)\) \{$/.exec(line)) {
      openBlock("REPEAT UNTIL ".concat(r.cond(m[1]), ":"));
      continue;
    }
    if (m = /^while \((.+)\) \{$/.exec(line)) {
      openBlock("WHILE ".concat(r.cond(m[1]), ":"));
      continue;
    }
    if (m = /^if \((.+)\) \{$/.exec(line)) {
      const taken = takeBlock(lines, i);
      out.push(pad + "IF ".concat(r.cond(m[1]), " THEN:"), ...stmts(r, taken.body, depth + 1));
      i = taken.next;
      if (taken.hasElse) {
...
function simple(r, line, pad) {
  ...
  if (m = /^\/\* (.*) \*\/$/.exec(text)) return ["".concat(pad, "# ").concat(m[1])];
  if (text.startsWith('//')) return ["".concat(pad, "#").concat(text.slice(2))];
  if (m = /^while \(!(.+)\)$/.exec(text)) return [pad + "wait until ".concat(r.cond(m[1]))];
  ...
  if (m = /^([A-Za-z_]\w*)\s*=\s*([^]+)$/.exec(text)) {
    ...
    if (/^bw_ask\(/.test(m[2].trim())) {
      ...
      return [pad + "ask ".concat(r.expr(call.args[0]), " and wait")];
    }
    return [pad + "set ".concat(target, " to ").concat(r.expr(m[2]))];
  }
  const call = asCall(text);
  if (!call) {
    r.warn("cannot read statement ".concat(text));
    return [];
  }
  const listOps = {
    bw_list_add: a => "add ".concat(r.expr(a[1]), " to ").concat(r.listName(a[0])),
    bw_list_delete: a => "delete ".concat(r.expr(a[1]), " of ").concat(r.listName(a[0])),
    bw_list_delete_all: a => "delete all of ".concat(r.listName(a[0])),
    bw_list_insert: a => "insert ".concat(r.expr(a[2]), " at ").concat(r.expr(a[1]), " of ").concat(r.listName(a[0])),
    bw_list_replace: a => "replace item ".concat(r.expr(a[1]), " of ").concat(r.listName(a[0]), " with ").concat(r.expr(a[2]))
  };
  if (listOps[call.name]) return [pad + listOps[call.name](call.args.map(stripCast))];
  if (call.name === 'bw_change') {
    return [pad + "change ".concat(r.listName(call.args[0]), " by ").concat(r.expr(call.args[1]))];
  }
  if (call.name === 'bw_wait') return [pad + "wait ".concat(r.expr(call.args[0]), " seconds")];
  ...
  const bare = (0,_sb3_creator_scratchruntime_js__WEBPACK_IMPORTED_MODULE_0__.stripSpritePrefix)(call.name).replace(/^do_/, '').replace(/_/g, ' ');
  r.warn("call to ".concat(call.name, " has no DEFINE marker"));
  return [pad + (call.args.length ? "".concat(bare, " ").concat(call.args.map(a => r.expr(a)).join(' ')) : bare)];
}
function stripCast(a) {
  return String(a).replace(/^\(int\)\s*/, '').trim();
}
