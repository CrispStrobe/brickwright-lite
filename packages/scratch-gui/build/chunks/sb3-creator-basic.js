"use strict";
(self["webpackChunkGUI"] = self["webpackChunkGUI"] || []).push([["sb3-creator-basic"],{

/***/ "./src/lib/sb3-creator-basic.js":
/*!**************************************!*\
  !*** ./src/lib/sb3-creator-basic.js ***!
  \**************************************/
...
/* harmony export */   "default": () => (/* binding */ basicToPseudocode)
/* harmony export */ });
/**
...
const UNMAPPED = new Set(['PLOT', 'COLOUR', 'COLOR', 'CLS', 'VDU', 'SOUND', 'ENVELOPE', 'DATA', 'READ', 'RESTORE', 'WHEN', 'OTHERWISE', 'ENDCASE', 'ON', 'OSCLI', 'CALL', 'CHAIN', 'LOCAL', 'CLEAR', 'PRINTTAB']);
function basicToPseudocode(source) {
  ...
  for (const raw of String(source || '').split(/\r?\n/)) {
    ...
    parts.push(cur);
    for (const p of parts) if (p.trim()) stmts.push({
      text: p.trim(),
      lineNo
    });
  }
    ...
    while (changed && guard++ < 200) {
      ...
      for (let k = 0; k < stmts.length; k++) {
        ...
        if ((m = t.match(/^IF\s+(.+?)\s+THEN\s+GOTO\s+(\d+)$/i)) && !/^NOT\s*\(/i.test(m[1])) {
          ...
          const j = stmts.findIndex((s2, kk) => kk > k && new RegExp("^GOTO\\s+".concat(stmts[k].lineNo, "$"), 'i').test(s2.text));
          if (j > k && idxOf(after) === j + 1 && countTargets(targets, stmts[k].lineNo) === 1 && countTargets(targets, after) === 1) {
            stmts[k] = {
              text: "WHILE NOT (".concat(m[1], ")"),
              lineNo: stmts[k].lineNo
            };
            stmts[j] = {
              text: 'ENDWHILE',
              lineNo: stmts[j].lineNo
            };
            changed = true;
            break;
          }
        ...
        if ((m = t.match(/^IF\s+NOT\s+\((.+)\)\s+THEN\s+GOTO\s+(\d+)$/i)) && Number(m[2]) !== stmts[k].lineNo) {
          ...
          if (e > k && countTargets(targets, Number(m[2])) === 1) {
            ...
            if (ge && idxOf(Number(ge[1])) > e && countTargets(targets, Number(ge[1])) === 1) {
              ...
              stmts[k] = {
                text: "IF ".concat(m[1], " THEN"),
                lineNo: stmts[k].lineNo
              };
              stmts[e - 1] = {
                text: 'ELSE',
                lineNo: stmts[e - 1].lineNo
              };
              stmts.splice(a, 0, {
                text: 'ENDIF',
                lineNo: null
  ...
  for (const p of pins) {
    ...
    pinByPoke.set("".concat(/^PB/i.test(p.where) ? 0 : 1, ":").concat(1 << bit), p);
  }
  ...
  const trExpr = e => {
    ...
    for (let pass = 0; pass < 4; pass++) {
      ...
      x = x.replace(/([A-Za-z_]\w*[%$]?)\(([^()]*)\)/g, (mm, name, idx) => {
        ...
        hit = true;
        return "(item ".concat(trExpr(idx), " of array \"").concat(vName(name), "\")");
      });
      if (!hit) break;
    }
    x = x.replace(/FN([A-Za-z_]\w*)\s*(\(([^()]*)\))?/gi, (mm, name, _p, argsText) => {
      ...
      if (!fn) {
        exprWarn.add("FN".concat(name, " has no single-line DEF FN \u2014 left as 0"));
        return '0';
      }
      let body = fn.body;
      ...
      fn.params.forEach((prm, k) => {
        var _args$k;
        body = body.replace(new RegExp("\\b".concat(prm.replace(/[%$]/g, '\\$&'), "\\b"), 'g'), "(".concat((_args$k = args[k]) !== null && _args$k !== void 0 ? _args$k : '0', ")"));
      });
      return "(".concat(body, ")");
    });
    x = x.replace(/RND\s*\(\s*([^)]+)\)/gi, 'pick random 1 to $1');
    // MID$(s, i, 1) → letter i of s.
    x = x.replace(/MID\$\s*\(\s*([^,]+)\s*,\s*([^,]+)\s*,\s*1\s*\)/gi, (_, s2, idx) => "letter ".concat(trExpr(idx), " of ").concat(trExpr(s2)));
    // LEN(s) → length of s.
    x = x.replace(/LEN\s*\(\s*([^)]+)\)/gi, (_, s2) => "length of ".concat(trExpr(s2)));
    // INSTR(a, b) > 0 → a contains b.
    x = x.replace(/\(INSTR\s*\(\s*([^,]+)\s*,\s*([^)]+)\)\s*>\s*0\)/gi, (_, a2, b2) => "".concat(trExpr(a2), " contains ").concat(trExpr(b2)));
    // INT(x+0.5) → round x.
    x = x.replace(/INT\s*\(\s*([^+]+)\+\s*0\.5\s*\)/gi, (_, a2) => "round ".concat(trExpr(a2)));
    // SQR(x) → sqrt of x.
    x = x.replace(/\bSQR\s*\(\s*([^)]+)\)/gi, (_, a2) => "sqrt of ".concat(trExpr(a2)));
    // ABS(x) → abs of x.
    x = x.replace(/\bABS\s*\(\s*([^)]+)\)/gi, (_, a2) => "abs of ".concat(trExpr(a2)));
    // INT(x) → floor of x.
    x = x.replace(/\bINT\s*\(\s*([^)]+)\)/gi, (_, a2) => "floor of ".concat(trExpr(a2)));
    // TIME/100 → timer.
    ...
    x = x.replace(/[A-Za-z_]\w*[%$]?/g, w => {
      ...
      if (/^(SIN|COS|TAN|LOG|LN|EXP|GET|VAL|PI|CHR\$|STR\$)$/.test(up)) {
        exprWarn.add("".concat(up, " has no dialect equivalent yet \u2014 kept textually"));
        return w;
      }
      return vName(w);
    });
  ...
  const trCond = c => {
    ...
    if (m) {
      const pin = pinByPoke.get("".concat(parseInt(m[1], 16) & 1, ":").concat(m[2]));
      if (pin) return "read ".concat(pin.name, " = ").concat(m[3]);
    }
    return trExpr(c);
  };
  ...
  const comment = s => line("# BASIC: ".concat(s));
  const named = s => {
    warnings.push("no dialect form for \"".concat(s.text, "\" \u2014 kept as a comment"));
    comment(s.text);
  };
  const procs = [];
  let i = 0;
  ...
  const bodyUsesVar = (v, from) => {
    for (let k = from; k < stmts.length; k++) {
      ...
      if (new RegExp("\\b".concat(v, "\\b"), 'i').test(stmts[k].text)) return true;
    }
    return false;
  };
  const pokeToPin = (port, exprText) => {
    ...
    if (m = e.match(/\bEOR (\d+)$/i)) {
      const pin = pinByPoke.get("".concat(port & 1, ":").concat(m[1]));
      if (pin) {
        line("toggle ".concat(pin.name));
        return true;
      }
    }
    if (m = e.match(/\bOR (\d+)$/i)) {
      const pin = pinByPoke.get("".concat(port & 1, ":").concat(m[1]));
      if (pin) {
        line(pin.activeLow ? "turn off ".concat(pin.name) : "turn on ".concat(pin.name));
        return true;
      }
    }
    if (m = e.match(/\bAND (\d+)$/i)) {
      const pin = pinByPoke.get("".concat(port & 1, ":").concat(255 - Number(m[1])));
      if (pin) {
        line(pin.activeLow ? "turn on ".concat(pin.name) : "turn off ".concat(pin.name));
        return true;
      }
    }
  ...
  const dispatch = s => {
    ...
    if (m = t.match(/^DEF\s+PROC([A-Za-z_]\w*)\s*(\(([^)]*)\))?$/i)) {
      ...
      procs.push({
        name: vName(m[1]).replace(/^_/, '').replace(/^do_/, ''),
        params,
        body
      });
      mapped();
    ...
    if (m = t.match(/^PROC([A-Za-z_]\w*)\s*(\(([^)]*)\))?$/i)) {
      ...
      line("".concat(vName(m[1]).replace(/^_/, '').replace(/^do_/, '')).concat(args.length ? ' ' + args.join(' ') : ''));
      mapped();
      return;
    }
    if (m = t.match(/^([a-z_]\w*)\s*=\s*TIME\+(.+)\*100$/i)) {
      const nxt = at(i + 1);
      if (nxt && new RegExp("^REPEAT UNTIL TIME>=".concat(m[1], "$"), 'i').test(nxt.text)) {
        line("wait ".concat(trExpr(m[2]), " seconds"));
        mapped();
        mapped();
        i++;
        return;
    ...
    if (m = t.match(/^\?&([0-9A-F]+)\s*=\s*(.+)$/i)) {
      if (pokeToPin(parseInt(m[1], 16) & 3, m[2])) {
        mapped();
        return;
      }
      named(s);
    ...
    if (m = t.match(/^DIM\s+([A-Za-z_]\w*[%$]?)\s*\(\s*(\d+)\s*\)$/i)) {
      ...
      line("new array \"".concat(vName(m[1]), "\" = [").concat(Array(Number(m[2]) + 1).fill(0).join(','), "]"));
      mapped();
      return;
    }
    if (m = t.match(/^DIM\s+/i)) {
      warnings.push("only 1D numeric DIM maps to the arrays extension so far \u2014 \"".concat(t, "\" kept as a comment"));
      comment(t);
      return;
    }
    if ((m = t.match(/^([A-Za-z_]\w*[%$]?)\s*\(([^()]*)\)\s*=\s*(.+)$/)) && dims.has(m[1].toUpperCase())) {
      line("set item ".concat(trExpr(m[2]), " of array \"").concat(vName(m[1]), "\" to ").concat(/^".*"$/.test(m[3].trim()) ? m[3].trim() : trExpr(m[3])));
      mapped();
      return;
    }
    if (m = t.match(/^CASE\s+(.+?)\s+OF$/i)) {
      ...
      while (i < stmts.length && !/^ENDCASE$/i.test(stmts[i].text)) {
        ...
        if (w = stmts[i].text.match(/^WHEN\s+(.+)$/i)) {
          ...
          branches.push({
            cond: vals.map(vv => "".concat(subj, " = ").concat(vv)).join(' or '),
            start: out.length
          });
      ...
      const emitChain = k => {
        ...
        line("IF (".concat(branches[k].cond, ") THEN:"));
        depth++;
    ...
    if (m = t.match(/^INPUT\s+(.+)$/i)) {
      ...
      if (prevMatch) {
        out[out.length - 1] = prevMatch[1] + "ask ".concat(prevMatch[2], " and wait");
      } else {
        line('ask "" and wait');
      }
      line("set ".concat(vr, " to answer"));
      mapped();
      return;
    }
    // CLG → clear (pen).
    ...
    // TIME=0 → reset timer.
    if (/^TIME\s*=\s*0$/i.test(t)) {
      line('reset timer');
      mapped();
      return;
    }
    // bw_pen%=TRUE/FALSE → pen down/up.
    if (/^bw_pen%\s*=\s*TRUE$/i.test(t)) {
      line('pen down');
      mapped();
      return;
    }
    if (/^bw_pen%\s*=\s*FALSE$/i.test(t)) {
      line('pen up');
      mapped();
      return;
    }
    // GCOL color → set pen color.
    if (m = t.match(/^GCOL\s+\d+\s*,\s*(.+)$/i)) {
      line("set pen color to ".concat(trExpr(m[1])));
      mapped();
      return;
    }
    // MOVE x,y → go to (pen-space coords: reverse the *4+640/512 transform).
    if (m = t.match(/^MOVE\s+(.+?)\s*,\s*(.+)$/i)) {
      line("go to x: ".concat(trExpr(m[1]), " y: ").concat(trExpr(m[2])));
      mapped();
      return;
    }
    ...
    if (m = t.match(/^PRINT\s+"([^"]*)"$/i)) {
      line("print \"".concat(m[1], "\""));
      mapped();
      return;
    }
    if (m = t.match(/^PRINT\s+([^;,'"]+)$/i)) {
      line("print ".concat(trExpr(m[1])));
      mapped();
      return;
    }
    if (m = t.match(/^FOR\s+([A-Za-z_]\w*[%$]?)\s*=\s*(.+?)\s+TO\s+(.+?)(\s+STEP\s+(.+))?$/i)) {
      const v = vName(m[1]);
      const from = trExpr(m[2]);
      const to = trExpr(m[3]);
      const step = m[5] ? trExpr(m[5]) : '1';
      const plain = from === '1' && step === '1' && !bodyUsesVar(m[1], i + 1);
      if (plain) line("REPEAT ".concat(to, ":"));else {
        line("set ".concat(v, " to ").concat(from));
        line("REPEAT ((".concat(to, ") - (").concat(from, ") + 1):"));
      }
      depth++;
      i++;
      walk(new RegExp("^NEXT(\\s+\\S+)?$", 'i'));
      if (!plain) line("change ".concat(v, " by ").concat(step));
      depth--;
      mapped();
      mapped();
      return;
    ...
    if ((m = t.match(/^REPEAT UNTIL\s+(.+)$/i)) && !/^TIME\s*>=/i.test(m[1])) {
      ...
      line("wait until ".concat(trCond(m[1])));
      mapped();
      return;
    }
    if (/^REPEAT$/i.test(t)) {
    ...
    if (m = t.match(/^WHILE\s+(.+)$/i)) {
      ...
      line("REPEAT UNTIL ".concat(inner ? trCond(inner[1]) : "not (".concat(trCond(m[1]), ")"), ":"));
      depth++;
      i++;
      walk(/^ENDWHILE$/i);
      depth--;
    ...
    if (m = t.match(/^IF\s+(.+?)\s+THEN$/i)) {
      line("IF (".concat(trCond(m[1]), ") THEN:"));
      depth++;
      i++;
      walk(/^(ELSE|ENDIF)$/i);
    ...
    if (/^GOTO\s+\d+$/i.test(t)) {
      warnings.push("unstructured \"".concat(t, "\" \u2014 restructure, or keep it as BASIC"));
      comment(t);
      return;
    }
    if (m = t.match(/^([A-Za-z_]\w*[%$]?)\s*=\s*(.+)$/)) {
      const v = vName(m[1]);
      const rhs = m[2].trim();
      const inc = rhs.match(new RegExp("^".concat(m[1].replace(/[%$]/g, '\\$&'), "\\s*\\+\\s*(.+)$")));
      if (inc) line("change ".concat(v, " by ").concat(trExpr(inc[1])));else line("set ".concat(v, " to ").concat(/^".*"$/.test(rhs) ? rhs : trExpr(rhs)));
      mapped();
      return;
    }
    named(s);
  ...
  // ---- assemble --------------------------------------------------------
  const head = ["DEVICE ".concat((device || 'EATER6502').toUpperCase())];
  for (const p of pins) head.push("PIN ".concat(p.name, " = ").concat(p.where.toUpperCase(), " ").concat(p.direction.toUpperCase()).concat(p.activeLow ? ' ACTIVE LOW' : ''));
  head.push('');
  for (const pr of procs) {
    head.push("DEFINE ".concat(pr.name).concat(pr.params.map(p => " (".concat(p, ")")).join(''), ":"));
    head.push(...pr.body);
    head.push('');
  }
