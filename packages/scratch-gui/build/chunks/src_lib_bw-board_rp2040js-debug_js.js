"use strict";
(self["webpackChunkGUI"] = self["webpackChunkGUI"] || []).push([["src_lib_bw-board_rp2040js-debug_js"],{

/***/ "./src/lib/bw-board/rp2040js-debug.js":
/*!********************************************!*\
  !*** ./src/lib/bw-board/rp2040js-debug.js ***!
  \********************************************/
...
/* harmony export */   createRp2040jsDebugTarget: () => (/* binding */ createRp2040jsDebugTarget)
/* harmony export */ });
/**
 ...
 */
function createRp2040jsDebugTarget(adapter) {
  ...
  function syncWriteTrap() {
    if (writeWatches.size && !origWrite8) {
      ...
      rp2040.writeUint8 = (a, v) => {
        checkWatch(a, v, 1);
        return origWrite8(a, v);
      };
      rp2040.writeUint16 = (a, v) => {
        checkWatch(a, v, 2);
        return origWrite16(a, v);
      };
      rp2040.writeUint32 = (a, v) => {
        checkWatch(a, v, 4);
    ...
    for (const task of sched.tasks || []) {
      ...
      for (const y of task.yields || []) {
        if (typeof y.addr === 'number') {
          yieldAddr.set("".concat(task.name, "/").concat(y.state), y.addr & ~1);
  ...
  function execute(untilNanos) {
    while (clock.nanos < untilNanos) {
      if (core.waiting) {
        ...
        const dt = toAlarm > 0 ? Math.min(toAlarm, untilNanos - clock.nanos) : untilNanos - clock.nanos;
        clock.tick(dt);
        continue;
      }
  ...
  const target = {
    ...
    setBreakpoint(bp) {
      ...
      if (bp.kind === 'code') {
        ...
        if ((bp.addr & 1) !== 0) {
          return {
            unsupported: "Thumb code addresses are halfword-aligned; bit 0 is the " + "execution-state flag, not part of the address: ".concat(bp.addr)
          };
        }
    ...
    writeMem(space, addr, data) {
      ...
      if (addr < RAM_START || addr + data.length > sramEnd) {
        return {
          refused: "write outside SRAM (0x".concat(RAM_START.toString(16), "..0x").concat(sramEnd.toString(16), ")")
        };
      }
