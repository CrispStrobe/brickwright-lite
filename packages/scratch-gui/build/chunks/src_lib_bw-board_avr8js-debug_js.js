"use strict";
(self["webpackChunkGUI"] = self["webpackChunkGUI"] || []).push([["src_lib_bw-board_avr8js-debug_js"],{

/***/ "./src/lib/bw-board/avr8js-debug.js":
/*!******************************************!*\
  !*** ./src/lib/bw-board/avr8js-debug.js ***!
  \******************************************/
...
/* harmony export */   createAvr8jsDebugTarget: () => (/* binding */ createAvr8jsDebugTarget)
/* harmony export */ });
/* harmony import */ var avr8js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! avr8js */ "./node_modules/avr8js/dist/esm/index.js");
/**
 ...
 */
function createAvr8jsDebugTarget(adapter) {
    ...
    for (const task of sched.tasks || []) {
      ...
      for (const y of task.yields || []) {
        if (typeof y.addr === 'number') {
          yieldAddr.set("".concat(task.name, "/").concat(y.state), y.addr);
          yieldSet.add(y.addr);
          yields++;
        }
      }
  ...
  function readVar(sym, fallbackSize) {
    ...
    const size = (_sym$size = sym.size) !== null && _sym$size !== void 0 ? _sym$size : fallbackSize;
    let v = 0;
    for (let i = size - 1; i >= 0; i--) {
      var _cpu$data;
      v = v << 8 | ((_cpu$data = cpu.data[sym.addr + i]) !== null && _cpu$data !== void 0 ? _cpu$data : 0);
    }
    return v >>> 0;
  }
  function positionOf() {
  ...
  function isCallOpcode(wpc) {
    var _cpu$progMem$wpc;
    const op = (_cpu$progMem$wpc = cpu.progMem[wpc]) !== null && _cpu$progMem$wpc !== void 0 ? _cpu$progMem$wpc : 0;
  ...
  function execute(untilCycles) {
    while (cpu.cycles < untilCycles) {
      ...
      (0,avr8js__WEBPACK_IMPORTED_MODULE_0__.avrInstruction)(cpu);
      cpu.tick();
      ...
      if (depthStep) {
        if (depthStep.kind === 'over') {
          if (!depthStep.entered) depthStep.entered = true;else if (readSP() >= depthStep.sp0) {
            running = false;
            depthStep = null;
            resumeGuard = cpu.pc * 2;
            syncBoard();
  ...
  const target = {
    capabilities() {
      return {
        ...
        writable: ['sram'],
        sfrs: 'memory-mapped',
        // AVR I/O registers live in the data space
    ...
    step(kind) {
      ...
      if (kind === 'block') {
        if (yieldSet.size === 0) {
          return {
            unsupported: symbols ? 'the symbol table has no yield addresses' : 'block stepping needs a symbol table (setSymbols first)'
          };
        }
        blockStep = true;
        insnRemaining = null;
      ...
      return {
        unsupported: "no such step kind: ".concat(kind)
      };
    },
    setBreakpoint(bp) {
      if (!bp || typeof bp !== 'object') return {
        unsupported: 'not a breakpoint'
      };
      if (bp.kind === 'code') {
        if (typeof bp.addr !== 'number') return {
          unsupported: 'code breakpoint needs addr'
        };
        if ((bp.addr & 1) !== 0) {
          return {
            unsupported: "AVR code addresses are even (byte address of a word): ".concat(bp.addr)
          };
        }
      ...
      if (bp.kind === 'yield') {
        const addr = yieldAddr.get("".concat(bp.task, "/").concat(bp.state));
        if (addr === undefined) {
          return {
            unsupported: symbols ? "no yield for ".concat(bp.task, "/").concat(bp.state, " in the symbol table") : 'yield breakpoints need a symbol table (setSymbols first)'
          };
        }
        const handle = nextHandle++;
        bps.set(handle, {
      ...
      if (bp.kind === 'write') {
        var _bp$len;
        if (typeof bp.addr !== 'number') return {
          unsupported: 'write watchpoint needs addr'
        };
        const handle = nextHandle++;
        bps.set(handle, {
          kind: 'write',
        ...
        writeWatches.set(handle, {
          addr: bp.addr,
          len: (_bp$len = bp.len) !== null && _bp$len !== void 0 ? _bp$len : 1
        });
        syncWriteHooks();
        return handle;
      }
      return {
        unsupported: "no such breakpoint kind: ".concat(bp.kind)
      };
    },
    ...
    readMem(space, addr, len) {
      ...
      if (space === 'code') {
        ...
        for (let i = 0; i < len; i++) {
          ...
          const word = (_cpu$progMem = cpu.progMem[byteAddr >> 1]) !== null && _cpu$progMem !== void 0 ? _cpu$progMem : 0;
          out[i] = byteAddr & 1 ? word >> 8 & 0xff : word & 0xff;
        }
        return out;
      }
      return {
        unsupported: "no such address space: ".concat(space)
      };
    },
    writeMem(space, addr, data) {
      if (space !== 'sram') return {
        refused: "space not writable: ".concat(space)
      };
    ...
    runFor(budgetNs) {
      ...
      const clockHz = (_adapter$clockHz = adapter.clockHz) !== null && _adapter$clockHz !== void 0 ? _adapter$clockHz : 16000000;
