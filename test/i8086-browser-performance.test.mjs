...
test('the production 8086 benchmark covers desktop and mobile pump health', () => {
    ...
    for (const fact of [
        ...
        'validateI8086WorkloadIntegrity', 'validateI8086ReceiptIdentity',
        'GITHUB_RUN_ID', 'GITHUB_SHA', 'rawReceipt', 'reportIdentityIssues', 'realTimeRatio',
...
test('raw and aggregate receipts fail closed on malformed or mismatched hosted identity', () => {
    const expected = {runId: 34063540552, headSha: 'a'.repeat(40)};
    const receipt = {evidence: {...expected}};
    assert.deepEqual(validateI8086ReceiptIdentity(receipt, expected), []);
    const mutations = [
        value => { delete value.evidence; },
        value => { value.evidence.runId = 0; },
        value => { value.evidence.runId = 1.5; },
        value => { value.evidence.headSha = 'short'; },
        value => { value.evidence.headSha = 'A'.repeat(40); }
    ];
    for (const mutate of mutations) {
        const value = structuredClone(receipt);
        mutate(value);
        assert.notDeepEqual(validateI8086ReceiptIdentity(value, expected), []);
    }
    assert.notDeepEqual(validateI8086ReceiptIdentity(receipt, {...expected, runId: expected.runId + 1}), []);
    assert.notDeepEqual(validateI8086ReceiptIdentity(receipt, {...expected, headSha: 'b'.repeat(40)}), []);
});
...
test('CPU-bound workload integrity fails closed on bypassed or mutated evidence', () => {
    ...
    const result = {
        workload: {
            ...
            heartbeatSegment: 0x1220,
            heartbeatAddress: 0x12310,
            heartbeatBefore: 10,
            heartbeatAfter: 30,
            heartbeatDelta: 20,
            cyclesBefore: 1000,
    ...
    const mutations = [
        ...
        value => { value.workload.heartbeatSegment++; },
        ...
        value => { value.realTimeRatio += 0.001; },
