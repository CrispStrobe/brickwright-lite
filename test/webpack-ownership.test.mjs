...
test('list monitor boundary enforces the measured 75 KiB initial reduction from P11', () => {
    const stats = lazyListFixture();
    stats.assets.find(asset => asset.name === 'gui.12345678.js').size = 4552523 - 9000 - 76799;
    const failures = assertLazyListMonitorBoundary(summarizeWebpackOwnership(stats));
    assert.match(failures.join('\n'), /reduction from P11 fell below 75 KiB: 76799 bytes/);
});
