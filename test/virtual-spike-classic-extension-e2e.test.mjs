...
test('bundled Classic extension uses corrected base64 RFCOMM end to end', async t => {
    ...
    assert.equal(hubState.data.lastPython, null, 'successful standard command must not eagerly send its fallback');
    await extension.setPixel({X: 2, Y: 3, BRIGHTNESS: 100});
    assert.equal(hubState.data.display[11], 9);
    assert.equal(hubState.data.lastPython, null, 'successful display command must not eagerly send its fallback');
...
test('bundled Classic wrapper records its immutable authoritative source', async () => {
    const pins = JSON.parse(await readFile(resolve(here, '../bundled-extension-pins.json'), 'utf8'));
    const pin = pins['CrispStrobe/legospikeprime_btc_scratchlink'];
    assert.equal(pin.repo, 'CrispStrobe/extensions');
    assert.match(pin.commit, /^[0-9a-f]{40}$/);
    assert.equal(pin.source, 'extensions/CrispStrobe/legospikeprime_btc_scratchlink.js');
    const wrapper = await readFile(resolve(here, `../${pin.overlay}`), 'utf8');
    assert.ok(wrapper.startsWith(
        `// Generated by scripts/sync-bundled-extensions.mjs from ${pin.repo}@${pin.commit}:${pin.source}.\n`
    ));
});
