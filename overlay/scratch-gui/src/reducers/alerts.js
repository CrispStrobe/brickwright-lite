 ...
 */
/**
 * Brickwright: show a standard alert that carries a MESSAGE.
 *
 * The reducer has always honoured `action.data.message` (it is how extension
 * alerts name the extension), but no action creator passed one, so every
 * standard alert could only show fixed text. An export that fails needs to say
 * WHY — a save that "just did nothing" is the shape of report nobody can act on,
 * and on iOS that is exactly what happened.
 *
 * @param {string} alertId - the alert to show
 * @param {string} message - detail appended by the alert component
 * @returns {object} an SHOW_ALERT action carrying the message
 */
const showStandardAlertWithMessage = function (alertId, message) {
    return {type: SHOW_ALERT, alertId, data: {message}};
};
