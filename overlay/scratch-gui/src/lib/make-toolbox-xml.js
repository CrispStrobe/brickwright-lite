...
const motion = function (isInitialSetup, isStage, targetId, colors) {
    const stageSelected = LazyScratchBlocks.get().ScratchMsgs.translate(
...
const looks = function (isInitialSetup, isStage, targetId, costumeName, backdropName, colors) {
    const hello = LazyScratchBlocks.get().ScratchMsgs.translate('LOOKS_HELLO', 'Hello!');
    const hmm = LazyScratchBlocks.get().ScratchMsgs.translate('LOOKS_HMM', 'Hmm...');
    // Note: the category's secondaryColour matches up with the blocks' tertiary color, both used for border color.
    return `
...
const sensing = function (isInitialSetup, isStage, targetId, colors) {
    const name = LazyScratchBlocks.get().ScratchMsgs.translate('SENSING_ASK_TEXT', 'What\'s your name?');
    // Note: the category's secondaryColour matches up with the blocks' tertiary color, both used for border color.
    return `
    <category
...
const operators = function (isInitialSetup, isStage, targetId, colors) {
    const apple = LazyScratchBlocks.get().ScratchMsgs.translate('OPERATORS_JOIN_APPLE', 'apple');
    const banana = LazyScratchBlocks.get().ScratchMsgs.translate('OPERATORS_JOIN_BANANA', 'banana');
    const letter = LazyScratchBlocks.get().ScratchMsgs.translate('OPERATORS_LETTEROF_APPLE', 'a');
    // Note: the category's secondaryColour matches up with the blocks' tertiary color, both used for border color.
    return `
    <category
