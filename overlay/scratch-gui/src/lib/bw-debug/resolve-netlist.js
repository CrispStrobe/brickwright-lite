// resolve-netlist.js -- ONE resolution of "which board is this?", for two callers.
//
// Extracted from debug-runner.js in 41d5f0cbb so the bare-metal debug path and
// the MicroPython Run resolve the board the same way rather than twice. It is
// load-bearing: the phantom-inferred-bench rejection (owner reports 2026-08-16
// and 2026-08-17) is this function's refusal.
//
// IT LIVED IN lib/bw-board/ UNTIL 2026-09-11, WHICH IS A VENDORED ROOT. Nothing
// in that root imported it -- both consumers are lite's own (bw-debug/debug-runner.js
// and pico-sim-run.js) and upstream has no counterpart -- so it sat where a sync
// operates, one careless run from being deleted with no upstream to restore it
// from. It is here now, beside the code that calls it.
// exists so that decision is asked rather than forgotten.
