...
// resolve-netlist.js is LITE-AUTHORED and lives inside a VENDORED root.
//
...
//
// WHY IT LIVES HERE:
// THE ONE WITH NO REASON TO BE HERE, and it is recorded rather than moved
// because moving it is a change to running code and this entry is not. NOTHING
// in the vendored root imports it: both consumers are lite's own --
// bw-debug/debug-runner.js and pico-sim-run.js -- and upstream has no
// counterpart. It was extracted from debug-runner.js in 41d5f0cbb so the
// bare-metal debug path and the MicroPython Run resolve the board the same
// way, and it landed in the vendored directory rather than beside either
// caller. It is load-bearing (the phantom-inferred-bench rejection, owner
// reports 2026-08-16/17), which is exactly why it should not be sitting where
// a sync operates. MOVING IT TO bw-debug/ IS A SEPARATE LANE; this entry
