...
export function buildSeatedFromDeclarations(circuit, stc, opts = {}) {
  ...
  // The controller is part of the physical circuit, not a generic placeholder.
  // Keep the STC package for Pn.m declarations, but use the audited board
  // sidecars for Arduino/Pico projects so their real header names and geometry
  // survive into seating, declarations, and the simulator netlist.
  ...
  if (!isBoard) circuit.seatPart(mcu.id, bb.id, computeLeadMap(FOOTPRINTS.mcu, 'e20'));
  ...
  // The controller itself is powered from the rails. Board power names come
  // from the sidecars: Uno/Nano use 5V, Pico uses VBUS, and the STC package
  // uses its explicit VCC/GND terminals.
