/**
 * Reduce the original built-in LED starter's rail topology to two direct taps.
 * This is deliberately fingerprinted: ordinary user circuits must not be
 * changed merely because they contain a battery, resistor and LED.
 */
...
export function migrateStarterAutosave(data) {
  ...
  const resistor = data.parts.find(part =>
    part.kind === 'resistor' && part.params?.ohms === 1000);
